...
namespace API_REST_With_DOTNET7.Repository.Implementations
{
    public class PessoaRepositoryImplementation : IPessoaRepository
    {
        ...
        // Teste
        public List<Pessoa> FindByIdadeRepository(int idade)
        {
            try
            {
                var result = _context.Pessoas.Where(x => x.Idade.Equals(idade.ToString())).ToList();
                ...
                {
                    throw new Exception("Erro: A idade pesquisada não consta no banco de dados!");
                }
            }
            catch (Exception ex)
            {
        ...
        //            return result;
        //        else
        //            throw new Exception("Erro: Id não econtrado! FindById");
        //    }
        //    catch (Exception ex)
        //    {
        //        _log.Info(ex);
